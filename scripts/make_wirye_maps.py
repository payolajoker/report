...
STOPS = [
  ...
  (37.490033, 127.153542, "북위례",        ""),             # OSM tram stop 확정
  (37.487768, 127.149364, "위례솔",        ""),             # OSM tram stop 확정
  (37.485322, 127.146259, "덕수고등학교",  ""),             # OSM 확정
  (37.480962, 127.143162, "위례호수공원",  ""),             # OSM 확정
  (37.477615, 127.142963, "위례별",        ""),             # OSM tram stop 확정
  (37.473586, 127.142168, "위례중앙광장",  "(장래 위례신사선)"), # OSM 확정
  # ── 분기점 ──
  (37.470567, 127.141962, "위례역사공원",  "분기"),         # OSM 확정  ← 본선+지선 공유
  # 지선 (보라): 역사공원 → 남위례
  (37.467522, 127.141772, "위례트램스퀘어",""),             # OSM tram stop 확정
  (37.464275, 127.139839, "남위례",        "8호선"),        # OSM tram stop 확정
  # 본선 계속 (파랑): 역사공원 → 복정
  (37.468928, 127.134778, "위례스마트시티",""),             # OSM tram stop 확정
...
_macheon = p(STOPS[0][0], STOPS[0][1])
_bukwirye = p(STOPS[1][0], STOPS[1][1])
m.add_line(Line([_macheon, _bukwirye], MAIN_C, 6))
m.add_marker(CircleMarker(_macheon,"white",22))
m.add_marker(CircleMarker(_macheon,XFER_C,15))
m.add_marker(CircleMarker(_bukwirye,"white",14))
m.add_marker(CircleMarker(_bukwirye,MAIN_C,9))

img = m.render(zoom=16, center=[127.153, 37.492])
draw = ImageDraw.Draw(img)
bx,by = 18,18
...
print("5/5 남위례역 근접…")
m = StaticMap(1000, 800, url_template=TILE)
_yeoksa   = p(STOPS[7][0], STOPS[7][1])
_tram_sq  = p(STOPS[8][0], STOPS[8][1])
_namwirye_tram  = p(STOPS[9][0], STOPS[9][1])
_namwirye_subwy = p(37.4627715, 127.1392162)  # 8호선 남위례역 OSM 확정
m.add_line(Line([_yeoksa, _tram_sq, _namwirye_tram], BRANCH_C, 6))
# 8호선 역사 (환승 기준점)
m.add_marker(CircleMarker(_namwirye_subwy,"white",22))
m.add_marker(CircleMarker(_namwirye_subwy,XFER_C,15))
# 트램 종착 정류장
m.add_marker(CircleMarker(_namwirye_tram,"white",14))
m.add_marker(CircleMarker(_namwirye_tram,BRANCH_C,9))
m.add_marker(CircleMarker(_tram_sq,"white",14))
m.add_marker(CircleMarker(_tram_sq,BRANCH_C,9))
m.add_marker(CircleMarker(_yeoksa,"white",14))
m.add_marker(CircleMarker(_yeoksa,"#888",9))

img = m.render(zoom=16, center=[127.140, 37.465])
draw = ImageDraw.Draw(img)
bx,by = 18,18
