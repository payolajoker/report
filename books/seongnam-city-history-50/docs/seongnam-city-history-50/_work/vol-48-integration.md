# Vol 48 Integration Note: 제48권 포토에세이, 성남 ALIGN
...
- source page marker와 `### p.xxx` 헤더는 본문 Markdown과 LLM 번들 각각 128개씩 있어야 한다.
- 48권은 통계 권차가 아니라 사진-캡션-정서 서사 중심 권차이므로, 병합 때 `stats`는 최소화하고 `figures`, 장소 색인, 교차 연결 태그를 중심으로 다루는 편이 좋다.
- 전체 페이지 이미지를 렌더링했으므로 포토에세이의 핵심 정보는 누락 없이 로컬 자산으로 보존되어 있다.
- 공용 색인 병합 시 `사회적 인프라`, `연결`, `성남 ALIGN`, `모인 사람들`, `공존의 자유`, `꿈꾸는 미래`, `하나되는 도시`, `행복한 삶`, `새로운 연결` 같은 주제 태그를 함께 두면 검색성이 좋아진다.
